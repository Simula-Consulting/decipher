...
def Compute_pos_last2(Pars):
    ...
    last2s = []
    for indx in range(nPatients_test):
        last2_z0 = last2_z(indx, 0, Pars, verbose=False)
        last2_z1 = last2_z(indx, 1, Pars, verbose=False)
        # print(last2_z0.sum(), last2_z1.sum())
        last2s.append([last2_z0, last2_z1])
    # print(last2s)
    print('Compute the predictive distribution of S*_I costs {}s'.format(time.time() - ts))
    return 0


def last2_z(indx, Z, Pars, verbose=False):
    Alpha = Pars[0][Z]
    Eta = Pars[1][Z]
    W = Pars[2][Z]
    C = Pars[3][Z]
...
def Compute_pos_last(Pars, verbose=False):
    ...
    Z = 0
    W0 = Pars[2][Z]
    MPmatrix0 = MPmatrixs[Z]
    Z = 1
    W1 = Pars[2][Z]
    MPmatrix1 = MPmatrixs[Z]

    for indx in range(nPatients_test):
        last2_z0, last2_z1 = last2s[indx]
        # print(last2_z0, last2_z1)
        patient_ages = ages_test[indx]

        # compute S_I+1|z=0, O*, hat_psi
        p_transition = ProbTransition(MPmatrix0, W0, patient_ages[-2], patient_ages[-1], inv)
        P_transition = p_transition[:-1, :-1]
        P_transition /= P_transition.sum(axis=1)[:, None]
        last_z0 = last2_z0.dot(P_transition)  # dim 2
        ...
        if verbose:
            print("{}th patient, last2_z0: {}, last_z0: {}, last2_z1: {}, last_z1: {}".format(indx, last2_z0, last_z0, last2_z1, last_z1))
        last = (1 - Z_pos[indx]) * np.concatenate([last_z0, np.zeros(2)]) + Z_pos[indx] * last_z1
        lasts.append(last)
