...
def ParVecShared2ParVec(parameter_vector_shared, n_inv, ind):
    ...
    if "Alpha" in ind:
        ...
        Alpha_vec = np.concatenate([Alpha0, Alpha1])
        parameter_vector = np.concatenate([parameter_vector, Alpha_vec])
    if "Eta" in ind:
        Eta_vec_shared = parameter_vector_shared[s: s+12]
        s += 12
        Eta1 = Eta_vec_shared
        Eta0 = Eta1[:6]
        Eta_vec = np.concatenate([Eta0, Eta1])
        parameter_vector = np.concatenate([parameter_vector, Eta_vec])
    if "W" in ind:
        W_vec = parameter_vector_shared[s: s+13*n_inv]
        s += 13 * n_inv
        parameter_vector = np.concatenate([parameter_vector, W_vec])
    if "C" in ind:
        C_vec = parameter_vector_shared[s: s+6*n_inv]
        s += 6 * n_inv
        parameter_vector = np.concatenate([parameter_vector, C_vec])
...
def ParVec2ParVecShared(parameter_vector, n_inv, ind):
    ...
    if "Alpha" in ind:
        ...
        parameter_vector_shared = np.concatenate([parameter_vector_shared, Alpha_vec_shared])
    if "Eta" in ind:
        Eta_vec = parameter_vector[s: s+18]
        s += 18
        Eta0 = Eta_vec[:6]
        Eta1 = Eta_vec[-12:]
        Eta_vec_shared = Eta1
        parameter_vector_shared = np.concatenate([parameter_vector_shared, Eta_vec_shared])
    if "W" in ind:
        W_vec = parameter_vector[s: s+13*n_inv]
        s += 13 * n_inv
        parameter_vector_shared = np.concatenate([parameter_vector_shared, W_vec])
    if "C" in ind:
        C_vec = parameter_vector[s: s+6*n_inv]
        s += 6 * n_inv
        parameter_vector_shared = np.concatenate([parameter_vector_shared, C_vec])
...
def ProbTransition(MPmatrix, W, start, end, inv: list[int]):
    '''
    ...
    while (temp < end):
        temp_component: int = Age2Comp(temp, inv)
        end_component: int = Age2Comp(end, inv)
...
def Loglikelihood_group_obs0(patient_tests, patient_observations, patient_ages, patient_treatment_status, patient_censor_age, patient_death_state, Z, Alpha, Eta, W, C, ind, inv, verbose = False, do_last = False):
    ...
    ### Initialization ###
    ### Q[s] ~ Pr[S0=s, O0]
    Q = np.zeros(nStates) # Transition intensities!
    patient_age = patient_ages[0]
    patient_test = patient_tests[0]
    patient_observation = patient_observations[0]
    if patient_treatment_status == 0: # so person has not been treated
        for s in range(nStates):
            Q[s] = np.log(ddirichlet_categorical(s,np.exp(C[:, Age2Comp(patient_age, inv)])))
            ...
            # P(S0, O0)
        Q = np.exp(Q) # then do all of this to calculate the intensities
    else: # so the patient has been treated: 100% probability of normal state
        Q[0] = 1
    log_Q = np.log(Q)
