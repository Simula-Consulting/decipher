...
# Converting to pts.
#age_partitions_pts = np.round((age_partitions - np.min(age_partitions)) * 4).astype(int)
...
def age_partitions_pts(points_per_year):
    ...
    return np.round((age_partitions - np.min(age_partitions)) * points_per_year).astype(int)

# Initial state probabilities: age group x probability initial state (adjusted to row stochastic).
...
def _start_end_times(N, time_grid, params, rnd=None):
    ...
    #p_cens = stats.exponweib.pdf(x=time_grid, a=513.28, c=4.02, loc=-992.87, scale=707.63)

    p_start = stats.lognomr(time_grid, s=params["lnorm_s"], loc=params["lnorm_loc"], scale=params["lnorm_scale"])
