...
def mcc_objective(thresh, y_true, y_pred_proba, clf):
    ...
    return -1.0 * matthews_corrcoef(y_true.astype(int), clf.predict(y_pred_proba).astype(int))
    

def estimate_proba_thresh(y_true, y_pred_proba, tol=1e-6, seed=42, n_thresholds=2):
    """Use differential evolution algorithm to estimate probability thresholds for the classification tree. 
    ...
    Returns:
        A ClassificationTree objecti instantiated with the estimated probaility thresholds. 
        This object may be saved to disk using scikit-learn routines. 
    """

    result = optimize.differential_evolution(mcc_objective, 
                                             bounds=optimize.Bounds([0] * n_thresholds, [1] * n_thresholds), 
                                             args=(y_true, y_pred_proba, ClassificationTree()), 
                                             seed=np.random.RandomState(seed=seed), tol=tol)
