...
class CMF(BaseMF):
	...
	def __init__(self, X, V, D=None, J=None, K=None, lambda1=1.0, lambda2=1.0, lambda3=1.0):
	...
	def _update_V(self):
		...
		hatV = ((self.Q2.T @ (self.S.T @ self.U + self.lambda2 * self.J)) @ Q1 / np.add.outer(self.L2, L1))
		self.V = self.Q2 @ (hatV @ Q1.T)

	def _update_U(self):

		self.U = np.transpose(np.linalg.solve(self.V.T @ self.V + self.I_l1, self.V.T @ self.S.T))
