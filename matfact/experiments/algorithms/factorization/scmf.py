...
class SCMF(BaseMF):
    ...
    def __init__(self, X, V, s_budget, W=None, D=None, J=None, K=None, lambda1=1.0, lambda2=1.0, lambda3=1.0, 
    ...
    def M(self):
        ...
        M = _take_per_row_strided(self.U @ self.V.T, start_idx=self.Ns - self.s, n_elem=self.T)
    ...
    def _init_matrices(self, D, J, K):
        ...
        self.X_bc = np.hstack([np.zeros((self.N, self.Ns)), self.X, np.zeros((self.N, self.Ns))])
        self.W_bc = np.hstack([np.zeros((self.N, self.Ns)), self.W, np.zeros((self.N, self.Ns))])

        self.V = np.vstack([np.zeros((self.Ns, self.r)), self.V, np.zeros((self.Ns, self.r))])
    ...
    def _approx_U(self):
        ...
        def _loss_U():
            ...
            return frob_loss + self.lambda1 * tf.reduce_sum(tf.square(tf.norm(U, axis=-1)))
    ...
    def _update_U(self):
        ...
        if self.n_iter_ > 0:
            ...
            for n in range(self.N):
                self.U[n] = self.X[n] @ self.V @ np.linalg.inv(self.V.T @ (np.diag(self.W[n]) @ self.V) + self.I1)

    def _update_s(self):

        # Evaluate the discrepancy term for all possible shift candidates 
        M = self.U @ self.V.T
        D = np.linalg.norm(self.W_shifts * (self.X_shifts - M[None, :, :]), axis=-1) ** 2
    ...
    def loss(self):
        ...
        loss = np.sum(np.linalg.norm(self.W * (self.X - self.U @ self.V.T), axis=1) ** 2)
